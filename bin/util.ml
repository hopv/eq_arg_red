...
module Option = struct
  ...
  let make f x = if f x then Some x else None
  let some_if = make
...
module List = struct
  ...
  let rec insert ?(cmp=Stdlib.compare) x xs =
    match xs with
    | [] -> [x]
    | x'::_ when cmp x x' <= 0 -> x::xs
    | x'::xs' -> x' :: insert x xs'
  ...
  module L = struct
    ...
    let fold_lefti ~f ~init xs = fold_lefti f init xs
    include Labels
  end
end
